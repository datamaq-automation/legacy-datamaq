...
- [x] Puertos/interfaces expl铆citos entre capas (avance implementado en migraci贸n vertical `contact`).
...
### Archivado desde `docs/todo.md` - 2026-02-27 (migraci髇 de estado)

- [x] `/api/v1/content`: Exponer `content_revision` en respuesta para trazabilidad de despliegues.
- [x] `/api/v1/content`: Validar metadatos base en contrato (`request_id`, `brand_id`, `version`) y m閠odo inv醠ido (`POST` -> `405`).

### Refactor `_bootstrap.php` por responsabilidades - 2026-02-27

- [x] Extraer CORS a m骴ulo dedicado.
  - Evidencia: `public/api/infrastructure/cors.php`.
- [x] Extraer security headers a m骴ulo dedicado.
  - Evidencia: `public/api/infrastructure/security_headers.php`.
- [x] Extraer request context (`request_id`, logging, timing) a m骴ulo dedicado.
  - Evidencia: `public/api/infrastructure/request_context.php`.
- [x] `_bootstrap.php` actualizado para componer m骴ulos extra韉os.
  - Evidencia: `public/api/_bootstrap.php` con `require_once` de infraestructura.
- [x] `/api/v1/content`: logging estructurado de versi髇 servido por request.
...
- [x] Decisi髇 t閏nica confirmada para `contact` + Chatwoot.
  - Evidencia: integraci髇 definida por Application API desde backend; no Public API desde frontend.

- [x] Separar responsabilidades de `_bootstrap.php` (cierre de bloque).
  - [x] Extraer validaciones a m骴ulo dedicado.
    - Evidencia: `public/api/infrastructure/request_validation.php`.
  - [x] Extraer rate-limit/config de rate-limit a m骴ulo dedicado.
    - Evidencia: `public/api/infrastructure/rate_limit.php`.
  - [x] Composici髇 final en bootstrap.
    - Evidencia: `public/api/_bootstrap.php` con `require_once` de m骴ulos de infraestructura (`cors`, `security_headers`, `request_context`, `request_validation`, `rate_limit`).
- [x] An醠isis de decisi髇 `contact` + Chatwoot formalizado (buenas pr醕ticas + DevOps).
  - Evidencia: `docs/todo.md` incluye certezas implementables y duda operativa expl韈ita sobre estrategia de inbox.
- [x] Integraci髇 inicial de `contact` con Chatwoot Application API en backend (multi-cuenta por marca).
